...
const SentimentResult: React.FC<SentimentResultProps> = ({ sentiment }) => {
  return (
    <div className="sentiment-result">
      <h4 style={{ color: getSentimentColor(sentiment) }}>{sentiment}</h4>
      <p>{getSentimentDescription(sentiment)}</p>
      <p>{getSentimentSuggestion(sentiment)}</p>
      <svg viewBox="0 0 24 24" style={{ fill: getSentimentColor(sentiment) }}>
